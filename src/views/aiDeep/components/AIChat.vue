...
const choose = (isExtend: boolean) => {
  ...
  if (userChoose.value) {
    ...
    messages.value.push({
      role: "system",
      content: `
你是一名经验丰富的 A 面试官，擅长通过候选人的岗位 JD 与简历内容挖掘其技术能力、项目实战经验与思维方式。

你的目标是：
- 基于候选人提供的岗位 JD 和简历内容，逐条进行深度提问与专业点评；
- 引导候选人展示其在项目中的实际贡献、技术能力、问题解决方式与职业素养。

你必须遵守以下规则：
- 每轮仅提出一个问题；
- 提问前必须对上一轮用户的回答进行一句简明专业的点评；
- 不得输出与点评或问题无关的内容（如寒暄、总结语等）；
- 问题必须基于用户提供的简历内容提出，避免泛泛而谈；
- 语言简洁、问题具体、具备深度，逐步推进。
...
你是本次对话的主导者，需持续提出有深度的问题，引导候选人不断展示自身能力。
`
    });

    message.value.role = "user";
    message.value.content = `面试官，您好，请读取我的这段项目/工作经历以及岗位jd！${'```resume\n' + resumeProject.value + '\n```'}  ${'```岗位jd\n' + jd.value + '\n```'}`;
    sendMessage();
    ...
    // });
    // message.value.role = "user";
    // message.value.content = `我现在求职的是${jd.value}岗位，\n  下面这个是我简历中的过往工作描述，我可以从哪些方面扩展优化？你的回复用文本就行，不需要md语法，给我一些思路：${resumeProject.value} `;
    // sendMessage();
    // messages.value.push({
    //   role: "system",
    //   content: "你是一个AI面试官，请根据用户输入的岗位jd以及工作/项目经历进行深度的项目拷打，你可以通过查看我的岗位jd和我的经历不断询问我问题，可以适当沿着项目延申询问或者深挖项目，可以追问我。你是主导者，要主导本次对话，不断向我进行面试提问，尽量一次只提问一个问题，然后用户回复后可以接着追问或者另外从另一个方面进行提问。"
...
const formatMessage = (content: string) => {
  // 处理<think></think>标签，替换为动画提示
  content = content.replace(/<think><\/think>/g, '<span class="ai-thinking"><span class="dot"></span><span class="dot"></span><span class="dot"></span> AI 正在深度思考...</span>');
  // 过滤掉```resume  ```中的内容
...
.ai-thinking .dot {
  ...
  background: #bbb;
  border-radius: 50%;
  animation: ai-thinking-blink 1.2s infinite both;
}
